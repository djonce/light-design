...
export default function Table(props: TableProps): JSX.Element {
  ...
  const theadTrClass = classNames({
    lightd_table_thead_th: true,
    lightd_table_thead_th_border: border,
    [`lightd_table_thead_th_size_${size}`]: size,
  });
  const tbodyTrClass = classNames({
    lightd_table_tbody_td: true,
    lightd_table_tbody_td_boder: border,
    [`lightd_table_tbody_td_size_${size}`]: size,
  });
  return (
    <div className={spaceClass} style={style || tableStyle}>
      <table className="lightd_table_content">
        <thead className="lightd_table_thead">
          <tr className="lightd_table_thead_tr">
        ...
        <tbody className="lightd_table_tbody">
          {dataSource && dataSource.length ? (
            dataSource.map((item: any, index: number) => (
              <tr className="lightd_table_tbody_tr" key={index + Math.random()}>
                {columns && columns.length
                      ...
                      return (
                        <td
                          className={[
                            tbodyTrClass,
                            column.ellipsis
                              ? 'lightd_table_tbody_td_ellipsis'
                              : '',
                          ].join(' ')}
                          key={index + i}
                          style={tdStyle}
