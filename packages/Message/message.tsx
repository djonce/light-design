...
el.className = 'lightd_message-container';
if (!document.querySelector('.lightd_message-container')) {
...
function MessageEle(props: MessageProps): JSX.Element {
  ...
  const spaceClass = classNames({
    lightd_message_item: true,
    [`lightd_message_item_${type}`]: type,
  });
  ...
  useEffect(() => {
    ...
    setTimeout(
      () => {
        ...
        setTimeout(() => {
          const container = document.querySelector('.lightd_message-container');
          container?.removeChild(
            document.querySelector('.lightd_message') as Node
          );
        }, 500);
      },
  ...
  return (
    <div>
      <CSSTransition
        ...
        unmountOnExit
      >
        <div className="lightd_message_position">
          <div className={spaceClass}>
            {type === 'loading' ? (
              <i className="m-icon-loading1 lightd_publicRotateEle" />
            ) : (
