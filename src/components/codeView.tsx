...
export default function CodeView(props: codeViewProps) {
  ...
  return (
    <>
      <div
        className="lightd-react-ui-codeview"
        style={{ height: openCode ? 'auto' : '0px' }}
      >
        <pre>
          <code>{code}</code>
        </pre>
        {showCopy ? (
          <div className="copyCode" onClick={copyCode}>
      ...
      {showView ? (
        <div
          className="viewCodeToggle"
          onClick={() => setOpenCode(!openCode)}
          style={{ borderTop: openCode ? 'none' : '1px solid #0000000f' }}
        >
          <span>{openCode ? '隐藏代码' : '显示代码'}</span>
